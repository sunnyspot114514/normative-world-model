...
- preregistration-v3 revision 1 只修改受控语言中的动作渲染及对应审计。其 smoke 语料在
  ...
  可读样本审查，以及无条件外部审计。
- 签字接受记录绑定了精确的 v3 smoke 语料、provenance manifest 和 29 文件 source
  lock。Phase-2 的 M1/M2 已用显式 parser 测试补齐。
- 正式 retained discovery 现包含每环境 1,000 个家族，并通过原生审计和包独立复算。
  retained Gate C 已在两个环境中分别通过：
...
- 外部接受仅解锁了 retained discovery；confirmation 仍为
...
- 精确的 `Qwen/Qwen3-1.7B-Base` revision 已缓存在项目内并记录逐文件哈希。联合臂
  one-step 的 725-token LoRA 单步优化在 RTX 3060 上通过，峰值 CUDA 分配为显存的
  66.7%。完整 rollout 目标依赖 WDDM 超额共享内存，已单独保留为资源失败诊断。
- retained Phase-1 语料已经在本地生成。confirmation 语料和正式三臂科学对照所需的
  adapter/checkpoint 尚未生成；当前 retained discovery adapter 仅包括下述窄范围
  schema gate 和已记为 BLOCKED 的工程 smoke。
- 随后的 schema-native anti-collapse smoke 已按冻结规则记为 **BLOCKED**：48 条平衡
  development 记录全部被预测为 `reject`，规范准确率为 `0.3333`，连续 event MAE 也差于
  零预测器。仅把训练增至 1024 步的 v2 虽解除单类塌缩，但仍为 **BLOCKED**（规范准确率
  0.2917；event MAE 0.5163，零预测器为 0.3534）。正式三臂对照没有启动。
...
scripts/
  ...
  verify-phase3-retained-schema-gate-result.py # 历史结果、adapter 与哈希验证器
...
- [Phase-3 内部 one-step pilot](docs/PHASE3_INTERNAL_PILOT.md)
...
- [v3 retained 执行合同](docs/PHASE1_V3_RETAINED_EXECUTION.md)
- [v3 retained discovery 记录](docs/PHASE1_V3_RETAINED_DISCOVERY.md)
...
- 具有冻结 provenance 和独立复算记录的 2,000 家族 retained Phase-1 语料
- 哈希锁定的 1.7B 本地基础模型与隔离 CUDA/PEFT 依赖栈
- 确定性 one-step 模型臂导出与禁止截断的 tokenizer 审计
- 匹配的 joint 多记录与 factorized 闭环探索性管线
...
- 已训练的语言模型 checkpoint
- retained 或可下结论的 joint 与 factorized 模型结果
- 模型层面的跨环境迁移结果
- Runtime 干预结果
- 关于通用价值或一般道德推理的主张
