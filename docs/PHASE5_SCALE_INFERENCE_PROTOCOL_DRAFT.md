...
The first population-deriving operation is the selector's first execution over the real retained-discovery export that emits any family ranking or selection. Selector implementation and fixture tests, renderer/verifier work, public tokenizer downloads, and synthetic tokenization probes are non-deriving. Stage 2 does not authorize that first real-source execution. After the local implementation and fixture review passes, a separate committed local-derivation gate must set `population_selection=true`; it authorizes exactly one deterministic selector execution over the named retained-discovery export and no remote action. The resulting real-source selection lock remains provisional until the Lock-A review accepts it; any selector change invalidates and re-derives the superseded lock.
...
The synthetic-only rental serves both checkpoints sequentially on the same GPU. It runs no selected project prompt and computes no scientific metric. The remote payload is governed by a fail-closed allowlist. It forbids project scenario files, selected prompt text, rendered-prompt caches, scientific request bodies, selected-prompt token-ID sequences, embeddings, base64 or other reversible encodings, and every other artifact derived from project prompt content. Only explicitly declared runtime/source files and public synthetic inputs may cross the boundary; local scientific hashes and counts remain local. Any undeclared file fails the transfer before rental work begins.
